...
print(paste('The Date with the highest number of transactions from Australia is', b['New_Invoice_Date'],'which is $',b['amount']))
# 4.e)
f=Online_Retail%>% group_by(New_Invoice_Hour)%>% summarise(Total.transaction= n())
n<-rollapply(f['Total.transaction'],2,sum)
index(min(n))
